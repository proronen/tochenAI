...
import styles from "./PostIdeasModal.module.css";
...
export default function PostIdeasModal({
    isOpen,
    onClose,
    onPostCreated,
}: PostIdeasModalProps) {
    ...
    const handleGenerateIdeas = async () => {
        if (!user?.business_description) {
            toast.showErrorToast(
                "Business description is required. Please set it in your client specifics.",
            );
            return;
        }
        ...
        try {
            ...
            const newIdeas = result.ideas.map(
    ...
    const handleDeleteIdea = (ideaId: string) => {
        setIdeas((prev) =>
            prev
                .map((idea) =>
                    idea.id === ideaId ? { ...idea, isActive: false } : idea,
                )
                .sort((a, b) => {
                            ...
                            {ideas.length > 0 && (
                                <Box>
                                    <Text fontWeight="semibold" mb={3}>
                                        Generated Ideas ({activeIdeas.length}{" "}
                                        active)
                                    </Text>
