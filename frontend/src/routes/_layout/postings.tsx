...
const defaultForm: Omit<UpcomingPostCreate, "scheduled_time"> & {
...
const PostingsPage = () => {
    ...
    const { generate } = Route.useSearch<{ generate: boolean }>();
    ...
    const handleToggle = async (
        id: string,
        field: "to_facebook" | "to_instagram" | "to_tiktok",
        value: boolean,
    ) => {
        setUpdating(id);
    ...
    const validateForm = () => {
        ...
        if (new Date(form.scheduled_time) < new Date())
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            const post = await createPosting({
                ...form,
                media_url: mediaUrl,
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        if (!editId) return;
        try {
            const updated = await updatePosting(editId, {
                ...form,
                media_url: mediaUrl,
                scheduled_time: new Date(form.scheduled_time).toISOString(),
            });
            setPosts((posts) =>
                posts.map((p) => (p.id === editId ? updated : p)),
            );
            setEditId(null);
            setIsEditOpen(false);
            setSelectedFile(null);
    ...
    const filteredPosts = posts.filter(
        (post) =>
            ...
            (filter.hashtag
                ? post?.hashtags
                      ?.toLowerCase()
                      .includes(filter.hashtag.toLowerCase())
                : true) &&
    ...
    if (loading && !generate) return <Spinner size="xl" mt={10} />;
        ...
        <Box p={6}>
                                ...
                                <DialogBody>
                                    <VStack gap={4} alignItems="stretch">
                                        ...
                                        <HStack>
                                            ...
                                            <SocialToggle
                                                icon={<FaTiktok />}
                                                isChecked={form.to_tiktok}
                                                onChange={() =>
                                                    setForm((f) => ({
                                                        ...f,
                                                        to_tiktok: !f.to_tiktok,
                                                    }))
                                                }
                                            />
                                        </HStack>
            ...
            <HStack gap={4} alignItems="center" mb={4}>
                ...
                <Input
                    ...
                    onChange={(e) =>
                        setFilter((f) => ({ ...f, hashtag: e.target.value }))
                    }
                    maxW="200px"
                />
                <HStack>
                    <SocialToggle
                        icon={<FaFacebook />}
                        isChecked={filter.facebook}
                        onChange={() =>
                            setFilter((f) => ({ ...f, facebook: !f.facebook }))
                        }
                    />
                    <SocialToggle
                        icon={<FaInstagram />}
                        isChecked={filter.instagram}
                        onChange={() =>
                            setFilter((f) => ({
                                ...f,
                    ...
                    <SocialToggle
                        icon={<FaTiktok />}
                        isChecked={filter.tiktok}
                        onChange={() =>
                            setFilter((f) => ({ ...f, tiktok: !f.tiktok }))
                        }
                    />
                </HStack>
            </HStack>
